...
def conectar_db():
    ...
    # Usuarios (Se añade la columna 'ciudad')
    cursor.execute('''
    ...
    # Código de seguridad por si la DB ya existía, añade la columna 'ciudad' si falta
    try:
...
# Lógica del Rango VIP según recargas aprobadas
...
def reg_foto(message):
    ...
    # Dejamos la descripción limpia y guardamos la foto final para activar la cuenta
...
# ==========================================
# 💬 CHAT PRIVADO MULTIMEDIA CON COBRO Y FILTRO
# ==========================================
...
def manejar_chat_texto(message):
    ...
    if texto == "🎁 Enviar Regalo":
        ...
        if pareja:
            ...
            # Traemos los datos de la cita con su ciudad
            ...
            # Contamos sus recargas aprobadas para calcular su nivel VIP
    ...
    enviar_multimedia_con_cobro(message, "text", bot.send_message, text=f"💬 **Tu cita:** {texto}", parse_mode="Markdown")
...
def manejar_chat_foto(message):
    ...
    if filtrar_contacto(caption):
        bot.send_message(message.chat.id, "🚫 **Acción bloqueada:** No puedes incluir un @usuario en la descripción de la foto.")
        return
    foto_id = message.photo[-1].file_id
    texto_final = f"📸 **Foto de tu cita**" + (f": {caption}" if caption else "")
...
def manejar_chat_video(message):
    caption = message.caption
    if filtrar_contacto(caption):
        bot.send_message(message.chat.id, "🚫 **Acción bloqueada:** No puedes incluir un @usuario en la descripción de este video.")
        return
    video_id = message.video.file_id if message.video else message.video_note.file_id
    if message.video:
        texto_final = f"🎥 **Video de tu cita**" + (f": {caption}" if caption else "")
...
# ==========================================
# 💰 RETIROS MÍNIMOS DE 25 DÓLARES REALES
# ==========================================
...
def controlador_menu_principal(message):
    ...
    if texto == "🔍 Buscar Citas":
        ...
        conn, cursor = conectar_db()
        # 1. Traemos los datos básicos del usuario, incluyendo su ciudad
        ...
        # 2. Contamos cuántas recargas reales tiene APROBADAS este usuario
        ...
        # 3. Asignamos el Rango VIP según sus recargas
        ...
        conn, cursor = conectar_db()
        query_top = """
            SELECT u.nombre, SUM(s.cantidad_monedas) as total_semanal
        ...
        txt_top = "🏆 **TOP 10 COMPRADORES** 🏆\n"
        txt_top += "⚡ _Los usuarios más exclusivos del bot_ ⚡\n"
        txt_top += "━━━━━━━━━━━━━━━━━━━━\n\n"
        
        if not top_usuarios:
            txt_top += "✨ ¡La competencia está abierta! Aún no hay recargas registradas. ¡Sé el primero en aparecer aquí! 🪙"
        else:
            medallas = ["🥇", "🥈", "🥉", "👑", "✨", "⚡", "💎", "⭐", "🔥", "📌"]
            for i, (nombre, total) in enumerate(top_usuarios):
                ...
                txt_top += f"└ 🪙 Recargado: `{total}` monedas\n\n"
                
        txt_top += "━━━━━━━━━━━━━━━━━━━━\n"
        txt_top += "🎯 _El Top se actualiza automáticamente cada vez que se aprueba un pago._"
        ...
        mensaje_ref = (
            ...
            f"Por cada amigo que invites al bot usando tu enlace, recibirás **50 monedas de regalo** 🪙 automáticas en tu saldo cuando terminen de工夫gur su tarjeta de perfil.\n\n"
