...
# 💰 TABLA DE TARIFAS DE INTERACCIÓN
COSTO_LIKE = 5                           # 💚 Costo por dar un Like común
COSTO_SUPERLIKE = 50                     # 💌 Costo por enviar un Super Like directo
COSTO_FILTRO_BUSQUEDA = 30               # 🎯 Costo por activar filtros de ciudad/edad
COSTO_RULETA = 25                        # 🎰 Costo por cada giro de la ruleta
COSTO_MENSAJE = 2                        # 💬 Costo por cada mensaje/multimedia enviado en el chat
MINIMO_RETIRABLE_MONEDAS = 2500          

# 🎁 CATÁLOGO EXCLUSIVO DE 20 REGALOS
CATALOGO_REGALOS = {
    1: ("🌹 Rosa Roja", 10),
...
# 🪙 MÁS DE 8 PAQUETES (9 EN TOTAL) EMPEZANDO BAJO ($2.000) HASTA ALTO
PAQUETES_MONEDAS = {
    "pack_40": {"monedas": 40, "precio": "$2.000 COP / $0.50 USD"},  
    "pack_100": {"monedas": 100, "precio": "$5.000 COP / $1.20 USD"},
    "pack_220": {"monedas": 220, "precio": "$10.000 COP / $2.50 USD"},
    "pack_350": {"monedas": 350, "precio": "$15.000 COP / $3.80 USD"},
    "pack_600": {"monedas": 600, "precio": "$25.000 COP / $6.00 USD"},
    "pack_1000": {"monedas": 1000, "precio": "$40.000 COP / $10.00 USD"},
    "pack_1300": {"monedas": 1300, "precio": "$50.000 COP / $12.50 USD"},
    "pack_2200": {"monedas": 2200, "precio": "$80.000 COP / $20.00 USD"},
    "pack_3500": {"monedas": 3500, "precio": "$120.000 COP / $30.00 USD"}
}
...
# ==========================================
# 📱 INTERFACES DE MENÚ PRINCIPAL Y CHATS
# ==========================================
def menu_principal():
...
def recibir_foto_registro(message):
    ...
    conn.commit()
    conn.close()
    bot.send_message(chat_id, "🎉 **¡Tu perfil VIP ha sido creado!** Te regalamos 50 monedas iniciales.", reply_markup=menu_principal())

# ==========================================
# 🎯 ENGINE DE CITAS PREMIUM CON FILTROS 
# ==========================================
...
def procesar_voto_swipe(call):
    ...
    if accion == "dislike":
        ...
        if not pago:
            conn.close()
            bot.send_message(chat_id, f"⚠️ Requiere `{COSTO_LIKE} monedas` para enviar un Like. Recarga saldo en el menú principal.")
            return
...
def procesar_botones_menu(message):
    ...
    if texto == "🔍 Buscar Citas":
        ...
        txt_tarjeta = f"👤 **Tu Tarjeta Comercial VIP:**\n━━━━━━━━━━━━━━━━━━━━\nID: `{chat_id}`\n📛 **Nombre:** {n}\n🎂 **Edad:** {e} años\n📍 **Ciudad:** `{c}`\n📝 **Bio:** _{d}_\n━━━━━━━━━━━━━━━━━━━━\n🪙 **Saldo:** `{m} 🪙` | 💰 **Regalos:** `{mr} 🪙`"
        ...
        markup = types.InlineKeyboardMarkup(row_width=1)
        # Bucle para renderizar las 9 opciones de precios (desde $2.000)
        for k, pack in PAQUETES_MONEDAS.items():
            markup.add(types.InlineKeyboardButton(f"🪙 {pack['monedas']} Monedas ({pack['precio']})", callback_data=f"buy_{k}_{pack['monedas']}"))
        bot.send_message(chat_id, "🪙 **CENTRAL DE RECARGAS VIP**\n\nSelecciona el paquete adquirido para reportar tu comprobante de pago:", reply_markup=markup)
...
def router_callbacks_global(call):
    ...
    if data == "toggle_filtro_ciudad":
        ...
        try: bot.send_message(target_id, f"🎁 **¡Has recibido un regalo virtual!**\n\nTe enviaron: {nombre_regalo}.\nSe han sumado `{ganancia_receptor} monedas` a tu balance acumulado para retiros.")
        except: pass

    elif data.startswith("buy_"):
        partes = data.split('_')
        monedas = int(partes[2])
...
# ==========================================
# 💬 INTERMEDIADOR DE CHAT PRIVADO MULTIMEDIA
# ==========================================
@bot.message_handler(content_types=['text', 'photo', 'sticker', 'video', 'voice'], func=lambda m: obtener_usuario_completo(m.chat.id)[0] == 'chateando')
def intermediador_chat_privado(message):
    ...
    # Validaciones de comandos de texto de los botones del chat activo
    ...
    # Cobro estructurado de 2 monedas por cualquier mensaje o multimedia enviado
    pago, _ = cobrar_monedas(chat_id, COSTO_MENSAJE)
    if pago:
        try:
            if message.content_type == 'text': 
                bot.send_message(receptor, f"💬 **Tu cita:** {texto}")
            elif message.content_type == 'photo': 
                bot.send_photo(receptor, message.photo[-1].file_id, caption="💬 **Tu cita te envió una foto**")
            elif message.content_type == 'sticker': 
                bot.send_sticker(receptor, message.sticker.file_id)
            elif message.content_type == 'video': 
                bot.send_video(receptor, message.video.file_id)
            elif message.content_type == 'voice': 
                bot.send_voice(receptor, message.voice.file_id)
        except: 
            pass
    else:
...
if __name__ == "__main__":
    print("🚀 Servidor VIP en línea con 9 opciones de recarga progresiva y soporte multimedia unificado...")
